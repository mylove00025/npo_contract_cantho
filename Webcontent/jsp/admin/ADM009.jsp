...
<html:html>
    ...
    <div id="navigator">
        <ul class="clearfix">
            ...
            <li>
                <span> > </span>
                <html:link href="#">Chỉnh sửa ngân hàng</html:link>
            </li>               
        </ul>
    </div>
    <%@ include file="/jsp/common/error_message.jsp" %>
    <html:form action="bankupdateview" styleId="form" onsubmit="javascript: save()">
        <html:hidden styleId="hasRelation" name="bankViewHelper" property="hasRelation"/>
...
<script type="text/javascript">
    ...
    function save() {
        document.bankForm.action="bankupdate.do";
        return true;
    }
    
    function remove() {
    	var hasRelation = document.getElementById("hasRelation").value;
        var answer;
        if (hasRelation == "false") {
            answer = confirm ("Bạn có thực sự muốn xóa dữ liệu?");
        } else {
            answer = confirm ("Ngân hàng này đang có dữ liệu hợp đồng liên quan. Nếu xóa thông tin ngân hàng sẽ có thể ảnh hưởng đến kết quả thống kê. "
            		   + "Bạn có thực sự muốn xóa dữ liệu?");
        }
        if (answer) {
	        document.bankForm.action="bankremove.do";
